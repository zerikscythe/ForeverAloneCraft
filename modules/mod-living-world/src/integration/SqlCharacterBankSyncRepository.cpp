...
bool SqlCharacterBankSyncRepository::SyncBankToCharacter(
    ...
    model::CharacterItemSnapshot const& cloneSnapshot)
{
    ...
    struct CloneBankCopy
    {
        model::CharacterItemSnapshotEntry item;
        ItemInstanceRow row;
    };

    std::vector<CloneBankCopy> cloneItems;
    cloneItems.reserve(cloneSnapshot.bankItems.size());
         ...
         cloneSnapshot.bankItems)
    {
        ...
        cloneItems.push_back(CloneBankCopy { item, *row });
    }
    ...
    for (CloneBankCopy const& copy : cloneItems)
    {
