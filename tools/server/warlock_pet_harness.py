#!/usr/bin/env python3
"""Spawn a Warlock world bot and verify summon + pet/glyph state."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--name", default="Warlockpetprobe")
    parser.add_argument("--spec", choices=sorted(SPEC_TO_KEY), default="demonology")
...
def spawn_temp_hostile(
    ...
    *,
    ...
    sql = (
        ...
        f"'warlock_pet_harness temp hostile from {source_guid}' "
        "FROM creature "
...
def main() -> int:
    ...
    identity_id = ensure_warlock_identity(
        settings,
        name=args.name,
        spec=args.spec,
        level=args.level,
    ...
    prefix = REPO_ROOT / f"warlock-pet-harness-{timestamp}"
