#!/usr/bin/env python3
"""Spawn a Hunter world bot and verify basic permanent-pet summon behavior."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--name", default="Hunterpetprobe")
    parser.add_argument("--spec", choices=sorted(SPEC_TO_KEY), default="beastmastery")
    ...
    parser.add_argument("--debug-bot-mode", choices=["assist", "passive", "hold", "stay", "guard"], default="assist")
    parser.add_argument("--hostile-source-guid", type=int, default=0)
    parser.add_argument("--hostile-offset-x", type=float, default=6.0)
    parser.add_argument("--hostile-offset-y", type=float, default=0.0)
    parser.add_argument("--hostile-offset-z", type=float, default=0.0)
    return parser.parse_args()


def ensure_hunter_identity(
...
def spawn_temp_hostile(
    ...
    *,
    ...
    next_guid = int(query_scalar(settings, "acore_world", "SELECT COALESCE(MAX(guid), 0) + 1 FROM creature"))
    sql = (
        ...
        "SELECT "
        f"{next_guid}, id1, id2, id3, {map_id}, zoneId, areaId, spawnMask, phaseMask, equipment_id, "
        f"{spawn_x}, {spawn_y}, {spawn_z}, orientation, spawntimesecs, 0, currentwaypoint, curhealth, curmana, "
        "MovementType, npcflag, unit_flags, dynamicflags, ScriptName, VerifiedBuild, CreateObject, "
        f"'hunter_pet_harness temp hostile from {source_guid}' "
        "FROM creature "
...
def delete_temp_hostile(settings: dict[str, str | int], guid: int) -> None:
    if guid <= 0:
        return
    run_mysql_query(settings, "acore_world", f"DELETE FROM creature WHERE guid = {guid}")
...
def build_report(
    ...
    focus = run_mysql_query(
        ...
        (
            ...
            f"WHERE id > {baseline_id} AND bot_guid = {identity_id} "
            "AND event_type IN ('pet_control', 'build_prepared', 'resource_snapshot', "
            "'glyph_snapshot', 'glyph_effect_snapshot', 'spellbook_snapshot', 'combat_enter', 'combat_exit') "
            "ORDER BY id ASC"
        ),
    )
...
def main() -> int:
    ...
    identity_id = ensure_hunter_identity(
        settings,
        name=args.name,
        spec=args.spec,
        level=args.level,
    ...
    temp_hostile_guid = spawn_temp_hostile(
        settings,
        source_guid=args.hostile_source_guid,
        map_id=args.map_id,
        spawn_x=args.spawn_x + args.hostile_offset_x,
        spawn_y=args.spawn_y + args.hostile_offset_y,
        spawn_z=args.spawn_z + args.hostile_offset_z,
    )

    timestamp = dt.datetime.now().strftime("%Y%m%d-%H%M%S")
    prefix = REPO_ROOT / f"hunter-pet-harness-{timestamp}"
    ...
    updates = {
        ...
        "LivingWorld.DebugForceBotModeIdentityId": str(identity_id if args.debug_bot_mode != "assist" else 0),
        "LivingWorld.DebugForceBotMode": args.debug_bot_mode,
    }
    ...
    try:
        ...
        delete_temp_hostile(settings, temp_hostile_guid)
